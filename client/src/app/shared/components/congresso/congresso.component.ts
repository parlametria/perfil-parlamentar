...
export class CongressoComponent implements AfterContentInit, OnChanges {
  ...
  draw(parlamentares: any[]) {
    ...
    for (let i = 0; i < 13; i++) {
      ...
      const circles = this.g.selectAll()
        ...
        .attr('stroke', 'none')
        .attr('cx', (d, i) => arc.node().getPointAtLength(x(i)).x)
        .attr('cy', (d, i) => arc.node().getPointAtLength(x(i)).y)
        .attr('opacity', 1)
  ...
  paint(parlamentares: Parlamentar[]) {
    this.g.selectAll('.circle-parlamentar')
      ...
      .duration(1000)
      .delay(100)
      .attr('opacity', (d) => (parlamentares.filter((p) => p.idParlamentar === d.idParlamentar).length > 0) ? 1 : 0.1);
  }
